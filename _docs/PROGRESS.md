...
- [x] Added a **National Science Week** promo card to `/events` (`events/page.tsx`) — a static card linking out to the Humanitix "Seeds of Science Festival" (Fri 21 Aug 2026, 4–7pm, free). Done as a code link rather than a DB event because the admin panel was unauthenticated in the automation browser. Note: Humanitix events do **not** get the live ticket count (that's Eventbrite-only).
- [x] Relabelled the Solar System price on `/events` from "From $15" to "Children's tickets only $15". Separately, the "45-minute" text in those event descriptions lives in the DB — updated via Supabase SQL (`UPDATE events SET description = REPLACE(...)`).
- [x] Homepage (`page.tsx`): added a "New shows announced →" pill just below the hero heading, linking to `/events`, with a premium **animated glowing gradient border** — `@property --pill-angle` + rotating `conic-gradient`, defined as `.glow-pill` in `globals.css`. Applied the same glow to the events-page "Get tickets" buttons, driven by two per-element CSS vars: `--glow-delay` (desynced animation phases so they don't pulse in unison) and `--glow-strength` (events buttons halved to 0.375 vs the hero's 0.75). Respects `prefers-reduced-motion`.
- [x] Added `_docs/CONTENT_MAP.md` — a map of where site content lives (GitHub / Vercel / Supabase / external services) plus a "where do I change X" cheat sheet.
